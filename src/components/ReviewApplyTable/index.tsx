...
const columns: ProColumns<DraftReviewItem>[] = [
  ...
  { title: '中期检查报告文件', dataIndex: 'file', width: 140 },
...
const ReviewApplyTable: React.FC = () => {
  ...
  return (
    <ProTable<DraftReviewItem>
      ...
      toolBarRender={() => [
        <Space>
          <Button type="primary" key="review" onClick={handleReview} disabled={selectedRows.length === 0}>
            审核
          </Button>
          <Button key="cancel-review" onClick={handleCancelReview} disabled={selectedRows.length === 0}>
            取消审核
          </Button>
        </Space>
      ]}
...
export default ReviewApplyTable;