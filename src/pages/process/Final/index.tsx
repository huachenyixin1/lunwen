...
const columns: ProColumns<FinalItem>[] = [
  ...
  { title: '论文终稿文件', dataIndex: 'file', width: 180, render: (text) => <a href="#" download>{text}</a> },
  ...
  { title: '终审状态', dataIndex: 'finalStatus', width: 100, render: (v) => v === '0' ? '审核通过' : '审核不通过' },
...
const Final: React.FC = () => {
  ...
  const [selectedRows, setSelectedRows] = useState<FinalItem[]>([]);
  ...
  return (
    <PageContainer>
      ...
      >
        ...
        >
          <Button icon={<UploadOutlined />}>上传论文终稿（.doc/.docx）</Button>
        </Upload>
      </Modal>
    </PageContainer>
  );
